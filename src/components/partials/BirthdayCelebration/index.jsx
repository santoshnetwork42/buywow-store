...
const BirthdayCelebration = React.memo(() => {
  ...
  useEffect(() => {
    // Check if balloons were already shown in this session
    const lastShownTime = localStorage.getItem("balloonsLastShown");
    const currentTime = new Date().getTime();
    const twentyFourHours = 24 * 60 * 60 * 1000; // 24 hours in milliseconds

    // If balloons were shown within last 24 hours, hide them
    if (
      lastShownTime &&
      currentTime - parseInt(lastShownTime) < twentyFourHours
    ) {
      const balloons = document.querySelectorAll(".balloons img");
      balloons.forEach((balloon) => {
        balloon.style.visibility = "hidden";
    ...
    // If we reach here, either balloons were never shown or it's been more than 24 hours
    localStorage.setItem("balloonsLastShown", currentTime.toString());

...
const AccountTabs = ({ tabItems, activeTab, handleTabClick }) => (
  <div className="no-scrollbar w-full shrink-0 overflow-x-auto md:flex">
    <div className="flex w-max flex-row sm:gap-2 md:flex-1 md:flex-col lg:gap-1">
      {tabItems.map((item) => (
        <AccountTabButton
          key={item.name}
          item={item}
...
const AccountTabButton = React.memo(({ item, isActive, onClick }) => (
  <Button
    onClick={onClick}
    size="large"
    className={`items-center justify-between rounded-md px-2.5 py-2 hover:bg-orange-500/50 ${isActive ? "bg-orange-500 font-medium hover:bg-orange-500" : ""} ${item.component === "Logout" ? "justify-start" : ""}`}
  >
    <Heading
      as="span"
      size="base"

...
const LoyaltyCash = ({
  ...
  if (!usableRewards) return null;

  return (
    <div className="mb-7">
      <div
        className={`flex justify-between rounded-lg border-[0.25px] border-[#DDDDDD] p-2.5 shadow-[0_4px_4px_#0000000D] md:p-3 ${!showLoyalty ? "pointer-events-none opacity-50" : ""}`}
      >
      ...
      {!showLoyalty && (
        <Text as="span" size="sm" responsive className="py-2 text-red-500">
          * WOW Cash cannot be clubbed with this offer
        </Text>
      )}
    </div>
  );

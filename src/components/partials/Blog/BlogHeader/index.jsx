...
export default async function BlogHeader() {
  ...
  return (
    <div className="container-main flex flex-col gap-y-6 border-b-[0.5px] border-solid border-gray-300_01 bg-white-a700_01 py-2.5 md:py-3 lg:py-4">
        ...
        <div className="hidden w-full items-center justify-center gap-x-4 lg:flex">
          {menu.map((item) => (
            <div key={item.id} className="group relative">
              <Link
                className="flex cursor-pointer items-center gap-x-1 py-2 hover:text-yellow-900"
                href={`/blog/${item.path}`}
              >
                {item.label}
